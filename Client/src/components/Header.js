	...
	return (
      ...
      <header className="items-center bg-white shadow-md md:flex px-32 justify-between hidden">
        ...
        <SearchBar title={"Search Projects ..."}/>
          ...
          <button onClick={() => navigate('/addproject')} className="p-2 rounded-md bg-primary text-white px-4">Submit Project</button>
          { !logged && (<div>
            <span onClick={() => navigate('/login')} className="text-lg font-medium">Sign In</span>
          </div>)}

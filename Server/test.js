...
module.exports = {
	addproject: async (req, res) => {
		...
		fileList.map((item) => {
			const fileScreenshot = base64ImageToBlob(item);
      const outputfile = `images/${Date.now()}.webp`
			sharp(fileScreenshot)
				.webp({ quality: 90 }) // Optional to change quality for more performance gainz
				.toFile(outputfile, (err, info) => {
					if (err) {
            uploadScreenshot(fileScreenshot)
					} else {
            uploadScreenshot(outputfile)
					}
				});
      ...
      sharp(file)
        ...
        .toFile(outputfile, async (err, info) => {
          if (err) {
            ...
            try{
              const thumbupload = await uploadBytes(storageRef, file); 
              thumbnailLink =  await getDownloadURL(thumbupload.ref);
            }catch(err){
            ...
            try{
              const thumbupload = await uploadBytes(storageRef, outputfile); 
              thumbnailLink =  await getDownloadURL(thumbupload.ref);
            }catch(err){
              res.json({status:false})
            }
